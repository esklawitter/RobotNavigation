; Main.asm
; Main control by Eric Klawitter
; Stub produced by Kevin Johnson
...
; subroutine for incrementing pointers to current target
LoadNext:
	; increment points for current target
	LOAD	CurTargetNum	; load targetX memory locationlocation
	ADDI	4		;increment curpost memloc by 3 (x,y,theta)
	STORE	CurTargetNum
	OUT	SSEG1
	
	LOAD	CurTargetX	; load targetX memory locationlocation
	ADDI	4		;increment curpost memloc by 3 (x,y,theta)
	STORE	CurTargetX
	Store	Curx
	
	LOAD	CurTargetY	; load targetY memory locationlocation
	ADDI	4		;increment curpost memloc by 3 (x,y,theta)
	STORE	CurTargetY
	ILOAD	CurTargetY
	Store	CurY
	
	LOAD	CurTargetTheta	; load targetY memory locationlocation
	ADDI	4		increment curpost memloc by 3 (x,y,theta)
	STORE	CurTargetTheta
	XOR	FinishAddr
	JZERO	Finish

	JUMP	QuickCheckPos	; jump to check position

	
; subroutine for checking position relative to target destiantion	
QuickCheckPos:
	IN	XPOS
	SUB	CurX
	CALL 	Abs
...
; subroutine called when robot has reached target destination
AtLoc:
	ILOAD	CurTargetNum		; load current target number
	OUT	SSEG2
	CALL	IndicateDest
	JUMP 	LoadNext		; ultimately log position/ play tone
	
;	when check postion subroutine is skipped, must calculate angle 	
CalculateAngle:
	IN	XPOS			; read in odometry x pos
	STORE	Temp1			; temporarily store
	ILOAD	CurTargetX
	SUB	Temp1			; subtract cur pos from target pos x	
	STORE	AtanX
	
	IN	YPOS			; read in odometry x pos
	STORE	Temp1			; temporarily store
	ILOAD	CurTargetY
	SUB	Temp1			; subtract cur pos from target pos x	
	STORE	AtanY

	IN	THETA			; read in theta
	STORE	Temp1			; store temporarily

	CALL	ATAN2			; calc vector angle
	SUB	Temp1			; theta - psi
	JNEG	NegAngle		; theta - psi < 0
	RETURN
	
NegAngle:
	ADDI	360			; angle error = theta - psi + 360
	RETURN
	
; calculate octant that the angle error falls into
Navigate:
	CALL	CalculateAngle
	STORE	Temp1
	SUB	XComp			; subtract x complement
	JNEG	COne			; jump to c3
	SUB	XTheta			; subtract x
	JNEG	CTwo			; jump to c1
	SUB	XTheta			; subtract x
	JNEG	CThree			; jump to c2
	SUB	XComp			; subtract x complement
	JNEG	CFour			; jump to c5
	SUB	XComp			; subtract x complement
	JNEG	CFive			; jump to c6
	SUB	XTheta			; subtract x
	JNEG	CTwo			; jump to c1
	SUB	XTheta			; subtract x
	JNEG	CThree			; jump to c2
	JUMP	Csix			; jump to c4
	
; subroutines for calculating wheel velocities based on angle error
; four subroutines for braitenberg control, 2 for turn in place
COne:

	LOADI	450
	OUT	RVELCMD		; left reverse max
	LOAD	Temp1
	SHIFT	2
	STORE	Temp2
	LOADI	450
	SUB	Temp2		
	OUT	LVELCMD		; right forward max
	JUMP	QuickCheckPos	; check pos
	
CTwo:
	LOAD	TurnPos
	OUT	RVELCMD		; left forward max
	LOAD	TurnNeg
	OUT	LVELCMD		; right reverse max
	JUMP	QuickCheckPos	; check pos

CThree:
	LOAD	TurnPos
	OUT	LVELCMD		; left forward max
	LOAD	TurnNeg
	OUT	RVELCMD		; right reverse max
	JUMP	QuickCheckPos	; check pos

CFour:
	LOADI	-450
	OUT	RVELCMD		; Right forward max
	LOAD	Temp1
	ADDI	-180
	SHIFT	2		; phi - 180 ) * 2 [negative val]
	STORE	Temp2
	LOADI	-450
	SUB	Temp2
	OUT	LVELCMD
...
CFive:
	LOADI	-450
	OUT	LVELCMD		; Right forward max
	LOAD	Temp1 		; load theta
	ADDI	-180
	SHIFT	2		; divide by four, set velocity accordingly
	ADDI	-450
	OUT	RVELCMD		; set as left velocity
	JUMP	QuickCheckPos	; check pos
...
ORG &H7CB
CurTargetNum: DW &H7CB
CurTargetX: DW &H7CC
CurTargetY: DW &H7CD
CurtargetTheta: DW &H7CE
DW 3
DW -870
DW -290
DW 0
